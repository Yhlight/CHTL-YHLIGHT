...
TEST(ParserTest, ParsesPropertyReference) {
    std::string source = "div { style { width: main.width; } }";
    CHTL::Lexer lexer(source);
    CHTL::Parser parser(lexer);
    ...
    ASSERT_EQ(prop->key, "width");
    ASSERT_EQ(prop->value.size(), 1);

    CHTL::PropertyReferenceNode* prop_ref = static_cast<CHTL::PropertyReferenceNode*>(prop->value[0].get());
    ASSERT_EQ(prop_ref->selector, "main");
    ASSERT_EQ(prop_ref->propertyName, "width");
}
