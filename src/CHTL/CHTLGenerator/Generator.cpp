        ...
        if (style_templates.count(node->name)) {
            ...
            for (const auto& provided_prop : node->provided_properties) {
                if (properties.count(provided_prop->key)) {
                    owned_properties.push_back(std::make_unique<StylePropertyNode>(provided_prop->key, provided_prop->getClonedValues()));
                    properties[provided_prop->key] = owned_properties.back().get();
                }
            }
