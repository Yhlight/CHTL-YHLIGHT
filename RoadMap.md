...
- [ ] **项目设置**
    - [ ] 初始化 CMake 构建系统
    - [ ] 配置 `.gitignore` 文件以排除构建产物和不必要的文件
    - [ ] 根据 `CHTL.md` 建议创建项目目录结构 (`CHTL/`, `CHTLJS/`, `Test/` 等)
- [ ] **基础工具库**
    - [ ] 实现字符串处理工具
    - [ ] 实现文件系统操作工具
    - [ ] 为所有工具库功能编写单元测试
- [ ] **CHTL 词法分析器 (Lexer)**
    - [ ] 定义所有 CHTL 词法单元 (Token)
    - [ ] 实现能够将 CHTL 源代码分解为词法单元序列的词法分析器
    - [ ] 编写全面的词法分析器测试
- [ ] **CHTL 语法分析器 (Parser) 与 AST**
    - [ ] 设计抽象语法树 (AST) 节点来表示语言的基本结构（元素、文本、属性）
    - [ ] 实现一个能够构建基础 AST 的递归下降语法分析器
    - [ ] 为核心语法结构编写单元测试
- [ ] **基础代码生成器 (Generator)**
    - [ ] 实现一个能够遍历 AST 并生成等效 HTML 的代码生成器
    - [ ] 编写针对代码生成的集成测试
- [ ] **构建与测试自动化**
    - [ ] 集成 Google Test 作为测试框架
    - [ ] 编写 Python 构建脚本以简化 `configure`, `build`, `test` 流程
...
- [ ] **模板系统 (`[Template]`)**
    ...
    - [ ] 实现模板的组合继承
- [ ] **原始嵌入 (`[Origin]`)**
    - [ ] 支持 `@Html`, `@Style`, `@JavaScript` 的原始代码嵌入
    - [ ] 支持带名原始嵌入
- [ ] **导入系统 (`[Import]`)**
